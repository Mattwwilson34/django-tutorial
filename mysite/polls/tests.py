...
class QuestionModelTests(TestCase):
    ...
    def test_was_published_recently_with_recent_question(self):
        """
        was_published_recently() returns True for quesitons whoe pub_date
...
def create_question(question_text, days):
    """
    Create a question with the given `questions_text` and published the given number of `days` offset to now
...
class QuestionsIndexViewTests(TestCase):
    def test_no_questions(self):
        """
        if no questions exist, an approppriate message is displayed.
        """
